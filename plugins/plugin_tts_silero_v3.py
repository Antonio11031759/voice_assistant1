...
def init(core:VACore):
    ...
    print("Silero TTS v3: Инициализация...")
    ...
    if not os.path.isfile(local_file):
        print("Silero TTS v3: Скачиваю модель...")
        torch.hub.download_url_to_file(modelurl, local_file)
        print("Silero TTS v3: Модель скачана успешно!")

    print("Silero TTS v3: Загружаю модель...")
    core.model = torch.package.PackageImporter(local_file).load_pickle("tts_models", "model")
    core.model.to(device)
    
    print(f"Silero TTS v3: Модель загружена на {device}")
    print(f"Silero TTS v3: Доступные голоса: {core.model.speakers}")
    print(f"Silero TTS v3: Текущий голос: {options['speaker']}")
    print(f"Silero TTS v3: Частота дискретизации: {options['sample_rate']}Hz")
    print(f"Silero TTS v3: Количество потоков: {options['threads']}")
...
def towavfile(core:VACore, text_to_speech:str, wavfile:str):
    ...
    print(f"Silero TTS v3: Озвучиваю текст: '{text_to_speech}'")
    ...
    if core.cur_callname != "":
        for k in options["speaker_by_assname"].keys():
            ...
            if core.cur_callname in ar_k:
                ...
                print(f"Silero TTS v3: Использую голос '{speaker}' для '{core.cur_callname}'")

    print(f"Silero TTS v3: Генерация аудио с голосом '{speaker}', частота {options['sample_rate']}Hz")
    ...
    print(f"Silero TTS v3: Аудио сохранено в {wavfile}")